...
class ChatTripPresenter(val view: ChatTripContract.View): ChatTripContract.Presenter, FirebaseDatabaseTripChatRepository.TripChatInteract {
    ...
    override fun sendMessage(message: String, uid: String) {
        if (message.trim().isNotEmpty()){
            val mes = Message().apply {userId = HablaveApplication.context.user?.uid!!; user = HablaveApplication.context.user?.nameAndSurname!!; idTrip = uid; this.message = message.trim(); createdAt = Calendar.getInstance().timeInMillis}
